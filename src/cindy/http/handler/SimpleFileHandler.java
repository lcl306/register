...
public class SimpleFileHandler extends SessionHandlerAdapter {
	...
	protected ByteBuffer getContent(String uri, File file)throws IOException{
		...
		if(file.isFile()){
			...
			buffer.append("<hl>Index of ").append(uri).append("</hl><hr/>");
	...
	public void objectReceived(Session session, Object obj)throws Exception{
		...
		if(content==null){
			...
			response.setParam("Content-Length", String.valueOf(NOT_FOUND.length));
			response.setContent(NOT_FOUND);
		}else{
			response.setStatusCode(200);
			response.setReasonPhrase("OK");
			response.setParam("Content-Type", mimeMap.getContentTypeFor(request.getRequestUri()));
			response.setParam("Content-Length", String.valueOf(content.remaining()));
			...
			if(content!=null)
				future = session.flush(new DefaultPacket(content));
			if(!keepAlive)
